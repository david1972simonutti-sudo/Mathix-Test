...
const HistoriqueExos = () => {
  ...
  const fetchChats = async (uid: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        `)
        .eq("user_id", uid)
        .eq("chat_type", "exercice") // Includes all exercise chats (with or without exercice_id)
        .order("created_at", { ascending: false });

      if (error) throw error;

      if (data) {
        // Pour chaque chat, compter les messages et filtrer ceux avec au moins 2 messages
        ...
        // Garder uniquement les chats avec au moins 2 messages (vraie conversation)
        const filteredChats = chatsWithCounts.filter(chat => chat.messageCount >= 2);
  ...
  const handleChatClick = async (chat: ChatItem) => {
    ...
    try {
      ...
      // 2. UNIQUEMENT si chat_history est VIDE, charger les interactions comme fallback (anciens chats)
      if (chat.exercices?.id && (!data || data.length === 0)) {
        ...
        if (intError) {
          ...
          // Mapper interactions en messages - remplace directement finalHistory
    ...
    try {
      ...
      // Si aucun champ reconnu, nettoyer et formater
        ...
        const technicalFields = [
          'type', 'error', 'requires_support', 'suggest_exercise',
        ...
        {view === 'list' ? (
          <>
            ...
            {isLoading ? (
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Aucun exercice pour le moment</CardTitle>
                  <CardDescription>
              ...
              <div className="space-y-4">
                {chats.map((chat) => {
                  const hasExercise = !!chat.exercices;
                  const { contexte, questions } = hasExercise 
                    ? parseEnonce(chat.exercices!.enonce)
                    : { contexte: '', questions: [] };
                    ...
                    >
                      <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-lg">
                          📚 {hasExercise ? (chat.titre || chat.exercices!.chapitre) : "Mon exo"}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        {hasExercise ? (
                          ...
                          <p className="text-muted-foreground text-sm">
                            Clique pour voir l'échange complet
                          </p>
                        )}
              ...
              {selectedChat && (
                <div className="space-y-2">
                  <p className="text-sm text-primary font-medium">
                    📚 {selectedChat.exercices ? selectedChat.exercices.chapitre : "Mon exo"}
                  </p>
